...
class TimeTableGenerator extends Model
{
    ...
    private function createInitialGeneration()
    {
        for ($y = 0; $y < $this->daysOfWeek; $y++) {
            for ($z = 0; $z < $this->sizeOfDay; $z++) {
                ...
                if ($z == $this->indexOfBreak) {
                    $indicator = $this->break[$y];
                    //print_r($indicator);
                    // $this->timeTable[$y][$z] = $this->break[$y];
                    // $this->timeTable[$y][$z] = $this->break[$y];
                }
                if ($this->hasLevelWideCourses()) {
                    // dd($this->hasLevelWideCourses());
                    if ($this->levelWideCourses[$y][$z] != '-'|| $this->levelWideCourses[$y][$z] != 'B' || $this->levelWideCourses[$y][$z] != 'R' ||$this->levelWideCourses[$y][$z] != 'E' || $this->levelWideCourses[$y][$z] != 'A' || $this->levelWideCourses[$y][$z] != 'K' ) {
                        $indicator = $this->levelWideCourses[$y][$z];
                               
                        
                    }
                    // print_r($this->levelWideCourses[$y][$z]);
                }
    ...
    {
        return $this->is_general == "1" ? false : true;
    }
            ...
            for ($y = 0; $y < $this->daysOfWeek; $y++) {
                for ($z = 0; $z < $this->sizeOfDay; $z++) {
                    ...
                    if ($this->hasLevelWideCourses() && $this->levelWideCourses[$row][$col] != '-' ) {
                        // dd($this->levelWideCourses[$z][$y]);
                        
                        $this->timeTable[$row][$col] = $this->levelWideCourses[$row][$col];
                        // print_r($this->timeTable[$row][$col]);
                        // dd($this->timeTable[$row][$col]);
                    } else {
                    ...
                    // dd($this->selectVenue($this->chromosomes[$x], $row, $col));
                }
            }
        }
        // dd($this);
    ...
    public function isBreakTimeZone($seed)
    {
        ...
        // dd(($seed - $this->sizeOfDay) % $this->indexOfBreak === 0);
